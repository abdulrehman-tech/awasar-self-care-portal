...
export default function Dashboard() {
  ...
  return <div className="space-y-6">
      ...
      {/* Promotions banner */}
      {promotions.length > 0 && <Link to="/catalog">
          <div className="gradient-primary rounded-lg p-4 text-white cursor-pointer hover:opacity-95 transition-opacity">
            <p className="font-semibold mx-0 my-0">{t(promotions[0].title, promotions[0].titleAr)}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        labelEn: "View Plans",
        labelAr: "عرض الباقات"
      }].map(action => <Link key={action.to} to={action.to}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentActivity.map(item => <div key={item.id} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
